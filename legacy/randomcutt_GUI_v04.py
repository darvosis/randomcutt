...
def extract_random_clips(input_file, num_clips, clip_duration, output_directory, codec="hap", audio=False):
    ...
    for i in range(num_clips):
        ...
        extension = "mov" if codec in ["prores_ks", "hap"] else "mp4"
        clip_name = f"{file_name}_clip_{i + 1}.{extension}"
        ...
        if codec == "prores_ks":
            cmd = [
                ...
                "-i", input_file,
                "-c:v", "prores_ks",
                "-profile:v", "3",  # or use "-c:v", "prores_ks -profile:v 3" directly
                "-qscale:v", "10",
                "-c:a", "pcm_s16le",
                clip_path
            ]
...
root.geometry("600x700")  # Set the initial size of the window
